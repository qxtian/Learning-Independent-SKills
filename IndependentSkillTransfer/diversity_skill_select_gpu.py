...
from Skill.diversity_skill import Diversity_skill
from Skill.independent_net import IcaNet, MLP
...
class Meta_control(object):
    def __init__(self, **kwargs):
        ...
        action_dim = self.skill_dim # self.env.action_space.shape[0]
...
if __name__ == "__main__":
    ...
    env_name = 'HalfCheetah_hurdle-v3'
    diversity_Skill = Diversity_skill(
        './Skill/diversity_skill/11281320-6skills-HalfCheetah-v3/11281320-6skills-HalfCheetah-v3_s16/ActorCritic.30000.pt')
    ...
    output_dir = './network_reserve/model_2323_diversity_select/'
    ...
    for episode_idx in range(MAX_EPISODE):
        ...
        for epoch in range(MAX_EPOCH):
            ...
            for step in range(MAX_STEP):
                # env.render()
                selection = meta_Control.act(state)
                # print(weight)
                action = diversity_Skill.select(state, selection)
                nxt_state, reward, done, info = env.step(action)
                meta_Control.replay_Buffer.push(state, selection, reward, nxt_state)
