...
class Logger:
    ...
    def log_tabular(self, key, val):
        ...
        """
        if self.first_row: # self.first_row turns to be False after logger.dump_tabular() executes
    ...
    def save_config(self, config):
        ...
        """
        del_key = ['vis', 'out_draw', 'temp_out_draw', 'out', 'out_j', 'out_c']
        for i in del_key:
            if i in config.keys():
                del config[i]
    ...
    def save_video(self, visual_obs, info, fpath):
        """
        info[video_prefix, ckpt_num, ep_ret, ep_len, con]
        """
        record_dir = osp.join(self.output_dir if fpath is None else fpath, 'video')
        os.makedirs(record_dir, exist_ok=True)
        if info[0]=='Trans' or 'Con':
            video_name = info[0]+'.'+info[1]+'.' +info[2] +'.mp4'
        else:
            [video_prefix, ckpt_num, ep_ret, ep_len, con] = info
            video_name = (video_prefix or 'V') + '.ckpt_{}.ret_{:.2f}.len_{}.{}.mp4'.format(
                        ckpt_num, ep_ret, ep_len, con)
        ...
        print("Video has been saved, dir = " + video_path)
