...
class HalfCheetahEnv_Hurdle_v3(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):

        self.interval_time = 0
        self.stage = 0
        mujoco_env.MujocoEnv.__init__(self, 'half_cheetah_hurdle_v3.xml', 5)
    ...
    def step(self, action):
        ...
        # if xposafter < x_destination - invalid_distance: # or yposafter > 3 or yposafter < -2.85: # or zposafter < -0.47:
        #     reward_failed = -800
        ...
        if xposafter > x_destination and yposafter < 3 and yposafter > -2.85: # the agent must stand
            reward_success = 1500
            reward = reward_ctrl + reward_run + reward_success + reward_failed + reward_height + reward_angle
            print('Success')
            return ob, reward, True, dict(reward_run=reward_run, reward_ctrl=reward_ctrl,
    ...
    def viewer_setup(self):
        self.viewer.cam.distance = self.model.stat.extent * 1 # 0.5
